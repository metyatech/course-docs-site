...
test("isolates hook Git variables without mutating its input", () => {
  ...
  for (const key of Object.keys(base).filter((key) => key.startsWith("GIT_")))
    assert.equal(isolated[key], undefined);
  assert.equal(isolated.PATH, base.PATH);
  assert.equal(isolated.HOME, base.HOME);
  assert.equal(isolated.NORMAL, "kept");
  assert.equal(base.GIT_DIR, "x");
});
...
test("isolated fixture env initializes a real temporary repository", () => {
  ...
  try {
    const result = spawnSync("git", ["init", dir], {
      encoding: "utf8",
      env: createIsolatedGitFixtureEnv({
        ...process.env,
        GIT_DIR: "outer",
        GIT_WORK_TREE: "outer",
      }),
