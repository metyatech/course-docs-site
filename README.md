...
each course. It builds the site for the configured `COURSE_CONTENT_SOURCE` and
then runs `verify:course:ci` (CI Playwright config). Use it when you need to
reproduce a CI matrix failure locally, or when validating changes that affect
course-specific build or E2E behavior. CI runs this command once per course
content source; reproduce a specific matrix entry by setting
