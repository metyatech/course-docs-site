...
| -------------------------- | ------------------------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run verify:content`   | Synced-content quality gate    | Every time you edit synced MDX / assets locally or before pushing content | Site-owned verifier that inspects root `content/` for Exercise heading rules and four-space code-block / asset indentation. Wired into `typecheck` / `build` / `build:verified` immediately after `sync:content`. |
